...
import static com.sjbt.sdk.BTConfig.SPP_UUID;
...
public class BtEngine {
    ...
    private BtEngine() {
        LogUtils.logBlueTooth("BtEngine() 构建");
    ...
    private static void socketConnectRead() throws IOException {
        ...
        if (!mSocket.isConnected()) {
            LogUtils.logBlueTooth("开始连接-->:" + mDevice);
        ...
        if (mSocket.isConnected()) {
            ...
            LogUtils.logBlueTooth("连接成功:" + device.getAddress());
...
//                            LogUtils.logBlueTooth("容许最大长度Transmit:" + socket.getMaxTransmitPacketSize());
                            ...
                            try {
                                ...
                                BtUtils.logBlueTooth("返回消息：" + msgStr);
...
//                        LogUtils.logBlueTooth("返回MSGCode：" + msgTimeCode);
...
//                        LogUtils.logBlueTooth("BIU APP 正常 消息队列：" + msgQueue.keySet());
                                parseMsg(result);
                                // 清空
                                result = new byte[0];
                            } catch (Exception e) {
    ...
    public static class MyHandlerThread extends HandlerThread {
        ...
        public void startThread() {
            LogUtils.logBlueTooth("startThread:启动线程");
            start();
            mBzyHandler = new Handler(getLooper()) {
                @Override
                public void handleMessage(Message msg) {

//                    LogUtils.logBlueTooth("handleMessage:" + msg.what);
                    // 在这里处理消息
                    switch (msg.what) {
                        case TYPE_MSG:
                        ...
                        case TYPE_CONNECT:
                            if (mDevice != null) {
                                LogUtils.logBlueTooth("创建BluetoothSocket：" + mDevice.getAddress());
                                try {
                                    mSocket = mDevice.createInsecureRfcommSocketToServiceRecord(SPP_UUID);
                                    LogUtils.logBlueTooth("开启子线程读取" + mDevice.getAddress());
                                    socketConnectRead();
                                } catch (IOException e) {
                                ...
                                LogUtils.logBlueTooth("设备异常Device=null");
                            }
                            break;
                    }
                }
        ...
        try {
            ...
            if (!notSetTimeOut(type, cmdId)) {
                ...
                msgQueue.put(msgTimeCode, new Runnable() {
                    @Override
                    public void run() {
                        LogUtils.logBlueTooth("Biu2Us消息发送超时回调：" + msgTimeCode);
...
//            LogUtils.logBlueTooth("开启子线程读取.容许最大长度Receive:" + mSocket.getMaxReceivePacketSize());
    ...
    public static void closeSocket(String name, boolean isNotify) {
        try {
            ...
            if (mSocket != null && mSocket.isConnected()) {
                mSocket.close();
                LogUtils.logBlueTooth(name + " 关闭Socket");
            ...
            public void run() {
                try {
                    if (mListener != null) {
                            ...
                            if (head == CmdConfig.HEAD_DEVICE_ERROR) {
                                ...
                                try {
                                    ...
                                    if (msgBean.cmdIdStr.equals(CmdConfig.CMD_STR_8015) && msgBean.head == HEAD_COMMON) {
                                        ...
                                        deviceBusing = (busy == 1);
                                        LogUtils.logBlueTooth("正在忙：" + deviceBusing);
                                        if (deviceBusing) {
                        ...
                        LogUtils.logBlueTooth("Listener 是NULL，消息无法下发");
                    }

                } catch (Throwable e) {
                    LogUtils.logBlueTooth("333.异常");
                    e.printStackTrace();
                }
            }
        });
...
//        LogUtils.logBlueTooth("第一条payLoad长度：" + payloadLen);
...
//            LogUtils.logBlueTooth("单独消息：" + BtUtils.bytesToHexString(msg));
...
//                LogUtils.logBlueTooth("payLoad2长度 hex:" + BtUtils.bytesToHexString(tempLenArray));
//                LogUtils.logBlueTooth("payLoad2长度：" + payloadLen);
...
//                LogUtils.logBlueTooth("摘开消息：" + BtUtils.bytesToHexString(tempMsg));
...
//                LogUtils.logBlueTooth("tempPosition：" + tempPosition);
            }
        }
    }
                ...
                public void run() {
                    try {
                        if (mListener != null) {
                            ...
                            LogUtils.logBlueTooth("BtEngine listener 被销毁");
                        }
                    } catch (Throwable e) {
                        e.printStackTrace();
                    }
